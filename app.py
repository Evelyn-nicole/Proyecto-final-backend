...
def availability():
    if request.method == "POST":
        ...
        reservation_date = Reservation.query.filter_by(date=date).first()
...
@app.route('/admin_edit_event/<int:id>', methods=["PUT"])
#@jwt_required()
def admin_edit_event(id):
    if request.method == "PUT":
        ...
        db.session.commit()
    return jsonify({
        "success": "Evento modificado con Exito"
    }), 200


# @app.route('/sendinfo', methods=["POST"])
# def sendInfo():
#     #date = Availability.query.filter_by(id=id).first()
#     #event = Availability.query.filter_by(event_id=event_id).first()
#     if request.method == "POST":
...
def admin_new_user():
    if request.method == "GET":
        Admin_user = Superadmin.query.all()
        results = list(map(lambda Admin_user: Admin_user.serialize(), Admin_user))
        return jsonify(results)
    ...
    if superadmin_exist != None:
        return jsonify("Usted ya existe como Super Administrador."), 404
    else:
